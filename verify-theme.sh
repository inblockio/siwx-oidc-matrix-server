...
#   3. config/element-theme-overrides.css still carries the settings-tab-label
#      rule and no longer carries the (removed, redundant) presence-dot rule.
#
# Usage:
...
# -- 3. Override CSS scope ----------------------------------------------------
if grep -q "mx_TabbedView_tabLabel_active" "$OVERRIDES"; then
  pass "override CSS keeps the settings-tab-label rule"
else
  fail "override CSS lost the settings-tab-label rule"
fi
if grep -q "mx_PresenceIconView_online" "$OVERRIDES"; then
  fail "override CSS still carries the removed online-presence-dot rule"
else
  pass "override CSS has no online-presence-dot rule (dot is green by construction)"
fi
