...
siwx-oidc-matrix-server/
  ...
  dockerfiles/
    Dockerfile             # FROM matrixdotorg/synapse; installs yq + patch; applies patches/synapse/*; COPYs entrypoints/matrix_server.sh
  patches/
    element-web/README.md  # registry of every Element Web deviation (why/evidence/retirement)
    synapse/README.md      # registry of every Synapse deviation (same discipline)
  entrypoints/
...
## Vendored Synapse patches

The Synapse image is **no longer stock**. `dockerfiles/Dockerfile` applies every
patch in `patches/synapse/` with `patch --forward --batch --fuzz=0`, so a patch
that stops applying **fails the image build** instead of silently shipping an
unpatched Synapse. `patches/synapse/README.md` is the registry — no patch ships
without an entry stating why, the evidence, its upstream status, and its
retirement condition. Run the bump procedure in that README **before** merging any
`FROM matrixdotorg/synapse:vX.Y.Z` change.

Today there is exactly one entry:

- **`msc4133-profile-field-write-policy.patch`** (UPSTREAM-TRACKED,
  [element-hq/synapse#19980](https://github.com/element-hq/synapse/pull/19980)) —
  adds `experimental_features.msc4133_key_{allow,deny}list` and a `403` guard for
  non-admin writes and deletes of listed custom profile fields. We need it because
  siwx-oidc publishes each user's DID into their MSC4133 profile under
  `io.inblock.did`, and on stock Synapse 1.159.0 that field is **freely
  user-writable with no value validation** — any user could overwrite their own
  copy with someone else's DID. `entrypoints/matrix_server.sh`
  (`apply_did_field_protection`) sets the denylist on every boot. Live A/B
  evidence: `siwx-oidc/docs/audits/2026-09-10-msc4133-acl-probe.md`.

  Note the guard **cannot** touch `displayname` or `avatar_url` — they route
  through `set_field` and never reach the guarded method. That is deliberate:
  `displayname` is the user's own alias and must stay user-owned. The identity
  model is three tiers — user-settable **alias**, derived **MXID**, provider-owned
  **DID** — and the patch enforces the boundary structurally.
