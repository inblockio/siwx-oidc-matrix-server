...
if [ -z "${LOCAL_DEV}" ]; then
    #federation listener with TLS (production only)
...
yq -i ".oidc_providers[0].issuer = \"${SIWXOIDC_BASE_URL}\"" /data/homeserver.yaml
...
if [ -n "${LOCAL_DEV}" ]; then
    # Allow HTTP OIDC issuer for local development (bypasses RFC 8414 HTTPS-only requirement)
    yq -i ".oidc_providers[0].skip_verification = true" /data/homeserver.yaml
fi

# Map did:pkh:eip155:1:0xabc... → @did_pkh_eip155_1_0xabc...:matrix.example.com
yq -i --unwrapScalar=false ".oidc_providers[0].user_mapping_provider.config.localpart_template=\"{{ user.sub | replace(':', '_') }}\"" /data/homeserver.yaml
